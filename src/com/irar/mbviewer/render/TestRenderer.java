...
public class TestRenderer implements IMBRenderer {
	...
	public void modifyIterData(HashMap<String, Double> iterData, Complex delta, Complex x, int curIter) {
		double dist = iterData.containsKey("distRunning") ? iterData.get("distRunning") : 0;
		double total = iterData.containsKey("total") ? iterData.get("total") : 0;
		dist += x.add(delta).mag();
		total += 1;
		iterData.put("distRunning", dist);
		iterData.put("total", total);
	}
	...
	private int getColor(Iteration iteration, Palette palette) {
		double colorloc = iteration.getExtraData().get("distRunning") / iteration.getExtraData().get("total");
		colorloc %= 1;
		int colorint = (int) (colorloc * palette.paletteloop.length);
		return palette.paletteloop[colorint];
	}
